...
# Load the model outside of any function
pipe_lr = joblib.load(open("models/emotion_classifier.pkl", "rb"))
...
def main():
	...
	custom_emotion = st.text_input("Enter your emotion:")
	if st.button("Predict Custom Emotion"):
		custom_prediction = predict_emotions(custom_emotion)
		st.success(f"Predicted Emotion: {custom_prediction}")

	# Batch Prediction
	batch_texts = st.text_area("Enter multiple texts (one per line):")
	if st.button("Predict Batch"):
		batch_predictions = [predict_emotions(text) for text in batch_texts.split('\n')]
		st.success("Batch Predictions:")
		st.write(batch_predictions)

		# Display Emojis for Batch Predictions
		emoji_batch_predictions = [emotions_emoji_dict[pred] for pred in batch_predictions]
		st.write("Emojis for Batch Predictions:")
		st.write(emoji_batch_predictions)
